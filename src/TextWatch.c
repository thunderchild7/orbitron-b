...
//	dayString = WEEKDAY_NAMES[language][pbltime->tm_wday];
  ...
  if (needToUpdateLine(&line1, line1Str, dayString)) {
    updateLineTo(&line1, line1Str, dayString);
  }
  if (needToUpdateLine(&line3, line3Str, dateString)) {
    updateLineTo(&line3, line3Str, dateString);
  }
}
...
{
  text_layer_set_font(textlayer, day_font);
...
void init() {
  ...
  window_set_background_color(window, GColorBlack );
    ...
    // FOnts
    date_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ORBITRON_LIGHT_15));	
    day_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ORBITRON_LIGHT_16));	
	time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_WWDIGITAL_BOLD_40));	


  // 1st line layers
  line1.currentLayer = text_layer_create(GRect(0, 39, 144, 22));
  line1.nextLayer = text_layer_create(GRect(144, 39, 144, 22));
  configureBoldLayer(line1.currentLayer);
  configureBoldLayer(line1.nextLayer);

  // 2nd layers
  line2.currentLayer = text_layer_create(GRect(0, 52, 144, 48));
  line2.nextLayer = text_layer_create(GRect(144, 52, 144, 48));
  configureLightLayer(line2.currentLayer);
  configureLightLayer(line2.nextLayer);

  // 3rd layers
  line3.currentLayer = text_layer_create(GRect(0, 94, 144, 20));
  line3.nextLayer = text_layer_create(GRect(144, 94, 144, 20));
  ...
  //layer_add_child(window_layer, (Layer*)day);
	...
	battery_layer = bitmap_layer_create(GRect(93, 3, 50, 20));
